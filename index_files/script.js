...
var END_SHOW_HREF         = "endshow.html",
    OUTLINE_EXPAND_HREF   = "outline_expanded.html",
    OUTLINE_COLLAPSE_HREF = "outline_collapsed.html",
    OUTLINE_NAVBAR_HREF  = "outline_navigation_bar.html",
    NAVBAR_HREF           = "navigation_bar.html",
    BLANK_NOTES_HREF	  = "blank_notes.html",
	...
	FS_NAVBAR_HREF		  = "fs_navigation_bar.html",
	isIEFiles 			= 0,
	isNAVFiles 			= 0,
	isFLATFiles 			= 16,
	includeNotes			= 0,
...
var g_baseURL = gtmpstr.substr(0, gtmpstr.lastIndexOf("/") ) + "/" + "index_files";
var g_showoutline = 1;
...
function FullScreen() {
	...
	SetFSMode(1);
	if ( msie >= 0 )
		FSWin = window.open( g_baseURL + "/" + "fullscreen.html", null, "fullscreen=yes");
	else {
		var height = screen.availHeight;
		if ( window.navigator.platform.indexOf( "Mac" ) >= 0 ) {
			height -= 30;
		}
		FSWin = window.open( g_baseURL + "/" + "fullscreen.html", "null", "height="+ height + ",width=" + screen.availWidth + ",screenX=0,screenY=0");
	}
}
  ...
  if ( (msie || isnav ) && ( (os < 0 && ver >= 4 ) || ( os >= 0 && ver >= 5 ) || (os >=0 && msie < 0 && ver >= 4 ) )  ){
	if ( msie >= 0 )  {
		if ( isIEFiles > 0 )
			...
			window.location.replace(  "index_files/error.html" );
	}
	else {
